...
# ==========================================
st.set_page_config(
    page_title="이우고(Yiwugo)-쿠팡 10배 마진 소싱 분석기",
    page_icon="🚀",
    layout="wide"
)

st.title("🚀 이우고(Yiwugo)-쿠팡 10배 마진 소싱 분석기")
st.markdown("""
이 도구는 **한국어 키워드**를 입력하면 **DeepL**을 통해 중국어로 자동 번역한 뒤, **이우고(Yiwugo.com)**에서 실시간 상품 정보를 수집합니다.
차단 걱정 없이 초저가 오프라인 도매 물건을 탐색하여 **쿠팡 10배 마진 상품**을 발굴합니다.
""")
...
def fetch_yiwugo_data(zh_keyword, token, limit=30):
    try:
        client = ApifyClient(token)
        run_input = {
            "searchKeyword": zh_keyword,
            "maxItems": limit
        }
        # Apify의 Yiwugo Scraper 액터 가동
        run = client.actor("janne/yiwugo-scraper").call(run_input=run_input)
        
        dataset_id = run.get("defaultDatasetId") if isinstance(run, dict) else run.default_dataset_id
        raw_items = list(client.dataset(dataset_id).iterate_items())
        return raw_items
    except Exception as e:
        st.error(f"이우고 데이터 수집 중 오류가 발생했습니다: {e}")
        return []
...
def analyze_margins(raw_items, target_price, exchange_rate, import_overhead, fee_rate, vat):
    ...
    for item in raw_items:
        ...
        price_raw = item.get("price") or item.get("unitPrice")
        
        if not price_raw:
            continue
        try:
            # 특수문자 제거 후 위안화 단가 숫자로 변환
            price_cny = float(str(price_raw).replace("¥", "").replace("$", "").strip())
        except ValueError:
            continue
        ...
        analyzed_list.append({
            ...
            "이우고 상품 링크": item.get("url") or item.get("detailUrl") or "https://www.yiwugo.com"
        })
...
# ==========================================
st.subheader("🔍 이우 도매시장 검색 및 마진 분석")
col1, col2, col3 = st.columns(3)
with col1:
    search_keyword = st.text_input("수집할 한국어 키워드", value="주방 아이디어 용품", help="찾고자 하는 상품군을 한국어로 적어주세요.")
with col2:
    target_price = st.number_input("쿠팡 판매 목표 가격 (원)", min_value=1000, value=25000, step=1000, help="쿠팡에 등록해 판매할 목표 소비자가격입니다.")
with col3:
    margin_threshold = st.number_input("필터링 기준 최소 마진 배수 (배)", min_value=1.0, value=10.0, step=0.5, help="입력한 수치 이상의 마진율을 확보한 상품만 걸러냅니다.")

st.markdown("---")

if st.button("🚀 이우고 마진 분석 실행", type="primary"):
    if not deepl_api_key or not apify_token:
        ...
        with st.spinner("🔄 이우 도매시장 정보 수집 중... 잠시만 기다려 주세요 (약 20초~40초 소요)"):
            # STEP 1: 번역
            zh_keyword = translate_ko_to_zh(search_keyword, deepl_api_key)
            if zh_keyword:
                st.info(f"🇨🇳 **중국어 번역 결과:** '{zh_keyword}' 키워드로 이우고를 검색합니다.")
                
                # STEP 2: 이우고 크롤러 가동
                raw_data = fetch_yiwugo_data(zh_keyword, apify_token, limit=30)
                if raw_data:
                    # STEP 3: 마진 분석
                    ...
                    st.success(f"🎉 분석 완료! 수집된 {len(df)}개 상품 중 {len(filtered_df)}개 아이템이 최소 {margin_threshold}배 마진 조건을 충족합니다.")
                    
                    tab1, tab2 = st.tabs(["📊 분석 결과 리스트", "📈 핵심 지표 분석"])
                    with tab1:
                        if not filtered_df.empty:
                            styled_df = filtered_df.copy()
                            styled_df["이우고 상품 링크"] = styled_df["이우고 상품 링크"].apply(lambda x: f'<a href="{x}" target="_blank">상품 바로가기</a>' if x else "")
                            st.write(
                            ...
                            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                                filtered_df.to_excel(writer, index=False, sheet_name='이우고_소싱_결과')
                            ...
                            st.download_button(
                                ...
                                data=processed_data,
                                file_name=f"yiwugo_margin_analysis_{search_keyword}.xlsx",
                                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
                            )
                        else:
                            st.warning(f"설정하신 최소 마진 배수({margin_threshold}배)를 확보할 수 있는 상품이 이우고 검색 결과에 존재하지 않습니다.")
                    with tab2:
                        if not filtered_df.empty:
